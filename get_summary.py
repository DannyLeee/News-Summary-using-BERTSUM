...
def get_summary(mode, encoder, origin_text):
    ...
    if mode == "ext":
        ...
        command = f"python3 /home/B10615023/PreSummWWM/src/train.py -task abs -mode test -report_rouge false -batch_size 3000 -test_batch_size 500 -bert_data_path {BERT_DATA_PATH} -log_file ./logs/inference -sep_optim true -use_interval true -visible_gpus 0 -max_pos 512 -alpha 0.95 -result_path {RESULT_PATH} -min_length 5 -max_length 25 -beam_size 10 -test_from /home/B10615023/PreSummWWM/model/model_step_500000.pt"
    os.system(command)
    ...
    with open(latest_file) as fp:
        return fp.readline()[:-1].replace(' ', '').split("<q>") # summary
